...
利用哪吒面板的计划任务，**30 秒**自动上报一次，所有探针持续在线，仅需一条命令。
...
- **替换密钥与 UUID**：务必把 `my-secret-key` 换成真实密钥，`uuid` 替换为你自己生成的 UUID。
- **国家代码**：支持 200+ 国家和地区，可自由搭配（如 `US`、`JP`、`FR`），请使用两位 ISO 代码。
- **探针数量**：⚠️ 注意单次请求最多 12 个；超过 12 个请拆分为多个计划任务。
- **执行频率**：计划任务 30 秒上报足以保持探针在线。若仍需更短间隔（如 20 秒）。
...
**Q：⚠️ 为什么单次限制 12 个？**  
为保证在免费计划的 CPU 时限内稳定完成。12 个探针串行上报约需 3～6 秒，处于安全范围内。
